  ...
  return (
    <main className="min-h-screen bg-ink text-ink-text">
      <header className="site-shell flex items-center justify-between py-6">
        <Link to="/" className="inline-flex items-center gap-2 text-sm text-ink-muted transition-colors hover:text-ink-text">
      ...
      <div className="site-shell pb-24 pt-12 sm:pt-20">
        <section className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-end">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:pb-2">
            ...
            <div className="panel p-6">
              ...
              <p className="text-sm text-ink-muted">Open to</p>
        ...
        <section className="mt-24 border-t border-ink-line pt-10">
          <div className="mb-10 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <p className="max-w-sm text-sm leading-6 text-ink-muted">Not the only work that matters, but the clearest signal of how I think and build.</p>
          </div>
            ...
            {proudProjects.map(({ label, project }) => (
              <a key={label} href={project.githubUrl ?? '#'} target="_blank" rel="noreferrer" className="panel group flex min-h-48 flex-col justify-between p-5 transition-transform hover:-translate-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Github size={16} className="text-ink-muted transition-colors group-hover:text-ink-text" />
                </div>
                <div>
                  <p className="font-semibold">{project.title}</p>
                  <p className="mt-2 text-xs leading-5 text-ink-muted">{project.tags.slice(0, 2).join(' · ')}</p>
                </div>
                <ArrowUpRight size={16} className="self-end text-ink-muted transition-colors group-hover:text-accent" />
              </a>
            ))}
          </div>
        </section>
        ...
        <section className="mt-24 grid gap-10 border-t border-ink-line pt-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <div className="panel grid gap-8 p-6 sm:grid-cols-[auto_1fr] sm:p-8">
            ...
            <div>
              ...
              <p className="mt-3 leading-7 text-ink-muted">I am currently building small systems that make AI workflows more inspectable: local-first model infrastructure, agent handoffs, and evaluation loops that turn experiments into durable practice.</p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-24 flex flex-col gap-6 border-t border-ink-line pt-10 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 text-sm text-ink-muted"><Sparkles size={16} className="text-accent" /> Available for thoughtful work.</div>
